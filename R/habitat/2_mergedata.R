...
# author:  Kingsley Griffin, Brooke Gibbons & Claude
# date:    July-Oct 2021
##
...
# Re running with data from regular formatting scripts
boss <- read.csv("data/Tidy/2021-05_Abrolhos_BOSS_random-points_broad.habitat.csv") %>%
  dplyr::mutate(method = "BOSS") %>%
  glimpse()
bruv <- read.csv("data/Tidy/2021-05_Abrolhos_BRUVs_random-points_broad.habitat.csv") %>%
...
allhab_sp <- SpatialPointsDataFrame(coords = hab[3:2], data = hab,
                                    proj4string = wgscrs)
...
allhab <- habi_df %>%
  dplyr::rename(kelps = broad.kelps,
                macroalgae = broad.macroalgae,
                sand = broad.unconsolidated,
                rock = broad.consolidated) %>%
  dplyr::mutate(biog = broad.sponges + broad.octocoral.black + broad.invertebrate.complex + 
                  broad.hydroids + broad.bryozoa + broad.ascidians) %>%
  glimpse()
...
saveRDS(allhab, "data/tidy/merged_habitat.rds")
