...
datn           <- nrow(habisp)
preddf         <- habi[, colnames(habi) %in% c("Z", "roughness", "tpi")]
...
modform        <- y ~ Intercept + depth + rough + tpi + f(sp, model = spde)
...
# predict back onto mesh from model given predictors
